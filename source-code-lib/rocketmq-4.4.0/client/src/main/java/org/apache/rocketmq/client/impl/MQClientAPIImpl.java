...
public class MQClientAPIImpl {
    ...
    private void pullMessageAsync(      //hLog 循环拉取任务
        final String addr,
        final RemotingCommand request,
        final long timeoutMillis,
        final PullCallback pullCallback
    ) throws RemotingException, InterruptedException {
        this.remotingClient.invokeAsync(addr, request, timeoutMillis, new InvokeCallback() {        //hLog broker交互
            @Override
            public void operationComplete(ResponseFuture responseFuture) {          //hLog 处理拉取消息
                RemotingCommand response = responseFuture.getResponseCommand();
                if (response != null) {
                    try {
                        PullResult pullResult = MQClientAPIImpl.this.processPullResponse(response);
                        assert pullResult != null;
                        pullCallback.onSuccess(pullResult);     //hLog 消息入队
                    } catch (Exception e) {
