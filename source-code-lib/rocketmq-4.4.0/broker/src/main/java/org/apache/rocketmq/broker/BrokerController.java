...
public class BrokerController {
    ...
    public void start() throws Exception {
        if (this.messageStore != null) {
            this.messageStore.start();      //hLog 消息存储服务启动，commitLog核心就在其中
        }

        if (this.remotingServer != null) {
            this.remotingServer.start();    //hLog server端启动
        }
        ...
        if (this.fileWatchService != null) {
            this.fileWatchService.start();  //hLog 监听密钥是否被移动？
        }

        if (this.brokerOuterAPI != null) {
            this.brokerOuterAPI.start();    //hLog broker客户端启动，一般是与nameServ 服务端交互
        }

        if (this.pullRequestHoldService != null) {
            this.pullRequestHoldService.start();    //hLog ？
        }
        ...
        if (this.filterServerManager != null) {
            this.filterServerManager.start();   //hLog filterServer 是个啥概念？
        }

        this.registerBrokerAll(true, false, true);  //hLog 向nameserver注册broker

        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {      //hLog 类似心跳检测，30s一次定时任务
        ...
        if (this.brokerStatsManager != null) {
            this.brokerStatsManager.start();        //hLog 空
        }

        if (this.brokerFastFailure != null) {
            this.brokerFastFailure.start();     //hLog 处理过期请求
        }

        if (BrokerRole.SLAVE != messageStoreConfig.getBrokerRole()) {       //hLog 主从配置？
            if (this.transactionalMessageCheckService != null) {
