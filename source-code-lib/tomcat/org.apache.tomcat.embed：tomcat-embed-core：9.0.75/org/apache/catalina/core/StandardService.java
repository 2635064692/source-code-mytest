...
public class StandardService extends LifecycleMBeanBase implements Service {        //service 主要实现类，包含server、connector、engine组件
     ...
     * Mapper listener.
     */
    protected final MapperListener mapperListener = new MapperListener(this);       //观察者模式，热部署监听
    ...
    protected void startInternal() throws LifecycleException {
        ...
        setState(LifecycleState.STARTING);      //service启动事件发送

        // Start our defined Container first
        if (engine != null) {
            synchronized (engine) {
                engine.start();                 //engine启动
            }
        }

        synchronized (executors) {
            for (Executor executor : executors) {
                executor.start();           //线程池初始化
            }
        }

        mapperListener.start();             //路由监听 （监听热部署的变化）
        ...
        synchronized (connectorsLock) {
            for (Connector connector : connectors) {
                ...
                if (connector.getState() != LifecycleState.FAILED) {
                    connector.start();      //连接器启动，内部启动完成后，才能对外提供服务
                }
            }
        }
    }
